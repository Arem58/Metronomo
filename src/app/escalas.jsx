...
const Escalas = () => {
  ...
  const handleClick = (event) => {
    ...
    if (id === 'Escalas#') {
      ...
      for (let i = 0; i < listNotas.length; i += 1) {
        ...
        if (calidadAcor[i * 2] === 'Tercera mayor' && calidadAcor[(i * 2) + 1] === 'Tercera menor') {
          const acordeActual = `Acorde de: ${listNotas[i]} ${listNotas[(i + 2) % size2]} ${calidadAcor[i * 2]} ${listNotas[(i + 4) % size2]} ${calidadAcor[(i * 2) + 1]}`
          + ' '
          + 'Acorde Mayor'
          + ' '
  ...
  return (
    <div>
      ...
      <button className="btn-change-path" type="button">
        <Link to="/" className="decorationA">
          {' '}
